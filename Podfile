# Uncomment the next line to define a global platform for your project
# platform :ios, '9.0'

target 'HiScore' do
  # Comment the next line if you don't want to use dynamic frameworks
  use_frameworks!

  # Pods for HiScore

# Lottie loads and renders animations and vectors exported in the bodymovin JSON format. Bodymovin JSON can be created and exported from After Effects with bodymovin, Sketch with Lottie Sketch Export, and from Haiku.
# https://github.com/airbnb/lottie-ios
pod 'lottie-ios'

# SkeletonView has been conceived to address this need, an elegant way to show users that something is happening and also prepare them for which contents are waiting.
# https://github.com/Juanpe/SkeletonView
pod 'Skeleton'

# IQKeyboardManager allows you to prevent this issue of keyboard sliding up and covering UITextField/UITextView without needing you to write any code or make any additional setup.
# https://github.com/hackiftekhar/IQKeyboardManager
pod 'IQKeyboardManager'


# A collection of loading buttons and their styling written in Swift.
# https://github.com/twho/loading-buttons-ios
pod 'MHLoadingButton'

# ButtonProgressBar is implement progress bar in button 
# https://github.com/thePsguy/ButtonProgressBar-iOS
pod 'ButtonProgressBar-iOS'

# NotificationBanner is an extremely customizable and lightweight library that makes the task of displaying in app notification banners and drop down alerts an absolute breeze in iOS.
# https://github.com/Daltron/NotificationBanner
pod 'NotificationBannerSwift', '~> 3.0.0'

# One Time Password View for iOS.
# https://github.com/Root-vb/OTPFieldView
pod 'OTPFieldView'

# SwiftLint is for code beautification.
# https://github.com/realm/SwiftLint
pod 'SwiftLint'

# This is use foe circular slider.
# https://github.com/HamzaGhazouani/HGCircularSlider
pod 'HGCircularSlider'

# Custom textField with error and placeholder with animation.
# https://github.com
pod 'DTTextField'

pod 'Kingfisher'
end
